...
import * as api_path from "../data/api_paths.json";
import {
  Amount,
  DateField,
  Field,
...
describe("Receipt Object initialization", async () => {
  ...
  it("should initialize from scratch", () => {
    ...
    expect((receipt.totalIncl as Amount).value).to.be.equal(params.totalIncl);
    expect((receipt.totalExcl as Amount).value).to.be.equal(params.totalExcl);
    expect((receipt.taxes as Tax[]).length).to.be.equal(params.taxes.length);
    expect((receipt.date as DateField).value).to.be.equal(params.date);
    expect((receipt.time as Field).value).to.be.equal(params.time);
    expect((receipt.merchantName as Field).value).to.be.equal(
      params.merchantName
    );
    expect((receipt.totalTax as Amount).value).to.be.equal(params.totalTax);
    for (const key in receipt.checklist) {
      expect(receipt.checklist[key]).to.be.true;
    }
  });
  ...
  it("should initialize from a prediction object with N/A value", async () => {
    ...
    expect((receipt.taxes as Tax[]).length).to.be.equal(0);
