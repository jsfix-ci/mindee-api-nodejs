...
describe("Test different types of input", () => {
  it("should accept base64 inputs", async () => {
    const b64String = await fs.readFile(
      path.join(__dirname, "data/documents/receipts/receipt.jpg"),
      { encoding: "base64" }
    );
  ...
  it("should accept files with a path", async () => {
    const input = new Input({
      file: path.join(__dirname, "data/documents/receipts/receipt.jpg"),
      inputType: "path",
    });
    await input.init();
  ...
  it("should accept read streams", async () => {
    const stream = createReadStream("data/documents/receipts/receipt.jpg");
    ...
    expect(input.fileExtension).to.equals("image/jpg");
    expect(input.fileObject).to.equals(stream);
  });
  ...
  it("should cut pdf", async () => {
    const input = new Input({
      file: path.join(__dirname, "data/documents/invoices/invoice_6p.pdf"),
      inputType: "path",
    });
    await input.init();
    const pdfDocument = await PDFDocument.load(input.fileObject as Buffer);
  ...
  it("should not cut pdf", async () => {
    const input = new Input({
      file: path.join(__dirname, "data/documents/invoices/invoice.pdf"),
      inputType: "path",
    });
    await input.init();
    const expectedResult = await fs.readFile(
      path.join(__dirname, "data/documents/invoices/invoice.pdf")
    );
    expect(input.inputType).to.equals("path");
